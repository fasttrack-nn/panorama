...
static void test_vs_recall(float *massQ, size_t vecsize, size_t qsize, HierarchicalNSW<float> &appr_alg, size_t vecdim,
               vector<std::priority_queue<std::pair<float, labeltype >>> &answers, size_t k, int adaptive, size_t ef_override) {
    vector<size_t> efs;
    if (ef_override > 0) {
        efs.push_back(ef_override);
    } else {
        efs.push_back(1500);
    }
    for (size_t ef : efs) {
...
int main(int argc, char * argv[]) {
    ...
    while(iarg != -1){
        iarg = getopt_long(argc, argv, "d:i:q:g:r:t:n:k:e:p:f:", longopts, &ind);
        switch (iarg){
            case 'd':
    ...
    test_vs_recall(Q.data, appr_alg->max_elements_, Q.n, *appr_alg, Q.d, answers, subk, randomize, ef_arg);
