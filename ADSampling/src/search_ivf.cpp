...
void test(const Matrix<float> &Q, const Matrix<unsigned> &G, const IVF &ivf, int k, int nprobe_override){
    float sys_t, usr_t, usr_t_sum = 0, total_time=0, search_time=0;
    struct rusage run_start, run_end;

    vector<int> nprobes;
    if (nprobe_override > 0) {
        nprobes.push_back(nprobe_override);
    } else {
    ...
    while(iarg != -1){
        ...
        switch (iarg){
            ...
            case 'p':
                ...
                break;
            case 'f':
                if(optarg)nprobe_arg = atoi(optarg);
                break;
            case 'i':
