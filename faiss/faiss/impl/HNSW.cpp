...
int search_from_candidates_panorama(
    ...
    // The panorama index stores rows interleaved as
    //   [cum_sums (n_levels+1 floats) | xb (d floats)]
    // inside the underlying IndexFlatPanoramaInline storage, so a single
    // cache-line fetch per visited candidate covers both the cum-sums
    // needed for pruning and the first level's worth of vector data
    // needed for the level-0 dot product. `cum_stride` (in floats) is
    // therefore the full row stride, not just (n_levels+1) like before.
    const size_t cum_stride = panorama_index->inline_row_floats();
    ...
    while (candidates.size() > 0 && !stop_flag) {
        ...
        // Pass 1+2 unioned over k_popped parents. Pass 1 just bounds
        // the per-parent neighbor count (terminator at v1 < 0); pass 2
        // (vt.set test-and-set) naturally dedupes neighbors that are
        // shared across parents in this K-batch, so no extra dedup
        // structure is needed.
        ...
        while (curr_panorama_level < num_panorama_levels && batch_size > 0) {
            ...
            const size_t start_dim = curr_panorama_level * level_width_dims;
            size_t end_dim = (curr_panorama_level + 1) * level_width_dims;
            ...
            // The base of the vector data inside the interleaved
            // IndexFlatPanoramaInline storage is `xb_base`; rows are
            // spaced `cum_stride` floats apart (= num_panorama_levels +
            // 1 + d), so we pass `cum_stride` as the stride to the
            // kernel rather than `d`.
            const float* level_base = xb_base + start_dim;

            with_level_width(dim_span, [&]<size_t W>() {
                compute_level_dot_products_flat<false, W>(
                        query + start_dim,
                        level_base,
                        index_array.data(),
                        batch_size,
