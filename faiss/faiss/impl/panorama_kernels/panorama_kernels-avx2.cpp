...
std::pair<uint8_t*, size_t> process_code_compression_impl<SIMDLevel::AVX2>(
    ...
    if (next_num_active < max_batch_size) {
        ...
        for (size_t point_idx = 0; point_idx < max_batch_size;
             point_idx += 64) {
            __m256i bs_lo = _mm256_loadu_si256(
                    (const __m256i*)(byteset + point_idx));
            __m256i bs_hi = _mm256_loadu_si256(
                    (const __m256i*)(byteset + point_idx + 32));
            __m256i bm_lo =
                    _mm256_cmpgt_epi8(bs_lo, _mm256_setzero_si256());
            __m256i bm_hi =
                    _mm256_cmpgt_epi8(bs_hi, _mm256_setzero_si256());
            uint64_t mask =
                    ((uint64_t)(uint32_t)_mm256_movemask_epi8(bm_hi) << 32) |
                    (uint64_t)(uint32_t)_mm256_movemask_epi8(bm_lo);
