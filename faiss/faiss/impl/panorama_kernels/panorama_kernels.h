 ...
 *
 * NOTE on `byteset`: the active-set buffer is a *byteset* (one byte per
 * element, value strictly 0 or 1), NOT a packed bitset. Historically it
 * was named `bitset`, but every writer assigns a `bool` and every reader
 * checks `!= 0`, so the per-byte LSB carries the only information.
 * Storing as bytes lets us cheaply derive a 64-byte 0xFF/0x00 lane mask
 * for byte-level stream compaction without a PEXT/PDEP roundtrip.
 */
...
/// Updates the byteset (one byte per element, 0/1) to reflect which
...
/// Byte-level stream compaction of PQ codes using the active byteset.
///
