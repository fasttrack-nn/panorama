...
std::pair<uint8_t*, size_t> process_code_compression_impl<SIMDLevel::AVX512>(
    ...
    if (next_num_active < max_batch_size) {
        ...
        for (size_t point_idx = 0; point_idx < max_batch_size;
             point_idx += 64) {
            __m512i bs_vec =
                    _mm512_loadu_si512((const __m512i*)(byteset + point_idx));
            uint64_t mask = (uint64_t)_mm512_cmpgt_epu8_mask(
                    bs_vec, _mm512_setzero_si512());
            ...
            for (size_t li = 0; li < level_width_bytes; li++) {
                ...
                for (int g = 0; g < 8; g++) {
                    ...
                    uint64_t compressed_val = _pext_u64(src_val, byte_masks[g]);
