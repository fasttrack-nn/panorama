...
struct PanoramaPQ : Panorama {
    ...
    /// @param byteset         Scratch buffer for code compression
    ///                         (one byte per element, value 0 or 1).
    ...
    size_t progressive_filter_batch(
        ...
        std::fill(byteset.begin(), byteset.end(), 0);
        size_t num_active = 0;
        ...
        for (size_t level = 0; level < n_levels && next_num_active > 0;
            ...
            // Pruning: compute lower bounds and decide which to keep.
            for (size_t i = 0; i < num_active_for_filtering; i++) {
                float exact_distance = exact_distances[i];
                float cum_sum =
                        cum_sums_level[active_indices[i] - batch_offset];
                float lower_bound =
                        exact_distance + coarse_dis -
                        (epsilon * cum_sum * query_cum_norm);
                byteset[active_indices[i] - batch_offset] =
                        (threshold > lower_bound) ? 1 : 0;
            }

            // Compacting: stream-compact active set based on decisions.
            size_t compacted = 0;
            for (size_t i = 0; i < num_active_for_filtering; i++) {
                bool keep =
                        byteset[active_indices[i] - batch_offset] != 0;
                active_indices[compacted] = active_indices[i];
                exact_distances[compacted] = exact_distances[i];
                compacted += keep ? 1 : 0;
            }

            next_num_active = compacted;
        }
