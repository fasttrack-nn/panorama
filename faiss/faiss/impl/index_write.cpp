...
void write_index(const Index* idx, IOWriter* f, int io_flags) {
    if (idx == nullptr) {
        ...
        WRITEVECTOR(idxpan->cum_sums);
            ...
            const IndexFlatPanoramaInline* idxinl =
                    dynamic_cast<const IndexFlatPanoramaInline*>(idx)) {
        // Serialize as a plain IxF2 storage holding *only* the vector
        // data (no cum-sums prefix). The owning `IndexHNSWFlatPanorama`
        // writes the cum-sums separately under the IHfP fourcc, and the
        // load path reconstructs the row-interleaved layout from the two
        // pieces. Keeping this on-disk shape identical to the legacy
        // format means previously serialized HNSW-Panorama indexes load
        // unchanged.
        uint32_t h = fourcc("IxF2");
        WRITE1(h);
        write_index_header(idx, f);
        const size_t cs_per_row = idxinl->cs_per_row;
        const size_t row_n = idxinl->row_floats();
        const size_t d_floats = static_cast<size_t>(idxinl->d);
        const size_t n_rows = static_cast<size_t>(idxinl->ntotal);
        // Gather the xb portion into a contiguous byte buffer so we can
        // reuse WRITEXBVECTOR and end up with the exact same wire shape
        // as a non-Panorama IndexFlat with this `xb` would produce.
        std::vector<uint8_t> tmp_xb(n_rows * d_floats * sizeof(float));
        const float* src =
                reinterpret_cast<const float*>(idxinl->codes.data());
        float* dst = reinterpret_cast<float*>(tmp_xb.data());
        for (size_t i = 0; i < n_rows; i++) {
            memcpy(dst + i * d_floats,
                   src + i * row_n + cs_per_row,
                   d_floats * sizeof(float));
        }
        WRITEXBVECTOR(tmp_xb);
        ...
        if (h == fourcc("IHfP")) {
            ...
            // The on-disk format stores cum-sums as a contiguous
            // (ntotal * (n_levels+1)) float vector to stay
            // backward-compatible with previously serialized indexes.
            // In-memory the cum-sums are interleaved with the vector
            // data inside the underlying IndexFlatPanoramaInline storage
            // (one cache line per [cum_sums | xb] row) for locality, so
            // we gather them out into a temporary contiguous buffer at
            // write time.
            const size_t cs_per_row =
                    static_cast<size_t>(idx_panorama->pano.n_levels + 1);
            std::vector<float> tmp_cum(
                    static_cast<size_t>(idx_panorama->ntotal) * cs_per_row);
            for (idx_t i = 0; i < idx_panorama->ntotal; i++) {
                memcpy(tmp_cum.data() + (size_t)i * cs_per_row,
                       idx_panorama->get_cum_sum(i),
