...
std::unique_ptr<Index> read_index_up(IOReader* f, int io_flags) {
    ...
    if (h == fourcc("null")) {
        ...
        // Holds the on-disk cum-sums vector for IHfP indexes until we
        // can interleave it with `storage->codes` once the storage has
        // been read further down. Empty for non-IHfP indexes.
        std::vector<float> ihfp_cum_sums_tmp;
        if (h == fourcc("IHfP")) {
            auto idx_panorama =
            ...
            // Read the legacy contiguous cum-sums layout into a temp;
            // we'll interleave it with the storage's vector codes into
            // a fresh IndexFlatPanoramaInline after the storage blob
            // has been deserialized below.
            READVECTOR(ihfp_cum_sums_tmp);
        }
        ...
        if (h == fourcc("IHfP")) {
            // The on-disk format embeds the storage as a plain IndexFlat
            // (just `xb`) plus a contiguous cum-sums vector. In memory we
            // want a single IndexFlatPanoramaInline whose `codes` already
            // hold rows in `[cum_sums | xb]` layout, so the search hot
            // path can read both kinds of data with one cache-line fetch.
            //
            // Naively we'd allocate a fresh IndexFlatPanoramaInline,
            // memcpy the just-loaded plain-IndexFlat into it, and free
            // the old one. That doubles the vector working set at peak
            // (~5GB extra at SIFT10M scale). Instead we *resize the
            // existing codes buffer in place* and shift each row's xb
            // floats forward by `cs_per_row` floats so we can prepend
            // its cum-sums in the freed slot, then re-tag the storage as
            // an IndexFlatPanoramaInline. This adds only ~120MB of
            // working memory (the `ihfp_cum_sums_tmp` buffer + the
            // `cs_per_row` floats appended at the tail), which is
            // released immediately after.
            auto* idx_panorama =
                    dynamic_cast<IndexHNSWFlatPanorama*>(idxhnsw.get());
            FAISS_THROW_IF_NOT_MSG(
                    idx_panorama,
                    "dynamic_cast to IndexHNSWFlatPanorama failed (rebuild)");
            auto* flat_storage =
                    dynamic_cast<IndexFlat*>(idx_panorama->storage);
            FAISS_THROW_IF_NOT_MSG(
                    flat_storage,
                    "IHfP storage must be IndexFlat for inline rebuild");
            FAISS_THROW_IF_NOT_MSG(
                    !dynamic_cast<IndexFlatPanorama*>(flat_storage),
                    "IHfP storage must NOT be a Panorama variant");
            const size_t cs_per_row = static_cast<size_t>(
                    idx_panorama->pano.n_levels + 1);
            const size_t d_floats = static_cast<size_t>(idx_panorama->d);
            const idx_t nt = idx_panorama->ntotal;
            FAISS_THROW_IF_NOT_FMT(
                    ihfp_cum_sums_tmp.size() == (size_t)nt * cs_per_row,
                    "IHfP cum_sums size %zu != ntotal*cs_per_row %zu",
                    ihfp_cum_sums_tmp.size(),
                    (size_t)nt * cs_per_row);

            // Allocate the new typed storage *empty*, just so we can
            // adopt the existing codes buffer and drop the old wrapper.
            auto* new_storage = new IndexFlatPanoramaInline(
                    idx_panorama->d, idx_panorama->pano.n_levels);
            new_storage->ntotal = nt;
            new_storage->is_trained = true;
            const size_t row_n = new_storage->row_floats();

            // Move the xb byte buffer out of the old storage.
            // `MaybeOwnedVector<uint8_t>` is move-assignable, so this
            // transfers ownership without an extra copy of the 5GB
            // payload.
            new_storage->codes = std::move(flat_storage->codes);
            // Now grow the codes buffer in place to hold the cum-sums
            // prefix of every row. `resize` only adds nt * cs_per_row *
            // sizeof(float) extra bytes (~120MB at SIFT10M scale).
            new_storage->codes.resize((size_t)nt * new_storage->code_size);

            // Walk rows from last to first, shifting each row's xb
            // floats forward by cs_per_row floats and writing the
            // corresponding cum-sums into the freed slot. Doing it from
            // the end means we never overwrite data we still need to
            // read.
            float* base = reinterpret_cast<float*>(new_storage->codes.data());
            for (idx_t i = nt - 1; i >= 0; i--) {
                float* row_dst = base + (size_t)i * row_n;
                // shift xb forward; memmove handles the
                // overlap when i is small.
                memmove(row_dst + cs_per_row,
                        base + (size_t)i * d_floats,
                        d_floats * sizeof(float));
                memcpy(row_dst,
                       ihfp_cum_sums_tmp.data() + (size_t)i * cs_per_row,
                       cs_per_row * sizeof(float));
            }

            delete idx_panorama->storage;
            idx_panorama->storage = new_storage;
            idx_panorama->own_fields = true;
        }
