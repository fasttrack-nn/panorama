...
struct IVFFlatScannerPanorama : InvertedListScanner {
    ...
    // Pre-allocated buffers reused across scan_codes calls to avoid
    // per-list heap allocation overhead (critical at high nprobe).
    mutable std::vector<float> exact_distances_;
    mutable std::vector<uint32_t> active_indices_;
    mutable std::vector<float> dot_buffer_;
    mutable std::vector<uint8_t> keep_mask_;
    ...
    IVFFlatScannerPanorama(
        ...
        exact_distances_.resize(pano_flat->batch_size);
        active_indices_.resize(pano_flat->batch_size);
        dot_buffer_.resize(pano_flat->batch_size);
        keep_mask_.resize(pano_flat->batch_size);
    }
    ...
    size_t scan_codes(
        ...
        const bool use_unopt = pano_flat->use_unoptimized_kernel;
        const bool disable_fw = pano_flat->disable_fixed_width;
        ...
        for (size_t batch_no = 0; batch_no < n_batches; batch_no++) {
            ...
            size_t num_active = with_metric_type(metric, [&]<MetricType M>() {
                if (use_unopt) {
                    return pano_flat
                            ->progressive_filter_batch_unoptimized<C, M>(
                                    codes,
                                    cum_sums_data,
                                    xi,
                                    cum_sums.data(),
                ...
                if (disable_fw) {
                    return pano_flat->progressive_filter_batch<
                            C, M, /*DisableFixedWidth=*/true>(
                            codes,
                            cum_sums_data,
                            xi,
                            cum_sums.data(),
...
void IndexIVFFlatPanorama::set_epsilon(float eps) {
    auto* storage =
            dynamic_cast<ArrayInvertedListsPanorama*>(invlists);
    FAISS_THROW_IF_NOT_MSG(
            storage,
            "IndexIVFFlatPanorama requires ArrayInvertedListsPanorama");
    storage->pano->epsilon = eps;
}

void IndexIVFFlatPanorama::set_use_unoptimized_kernel(bool v) {
    auto* storage =
            dynamic_cast<ArrayInvertedListsPanorama*>(invlists);
    FAISS_THROW_IF_NOT_MSG(
            storage,
            "IndexIVFFlatPanorama requires ArrayInvertedListsPanorama");
    storage->pano->use_unoptimized_kernel = v;
}

void IndexIVFFlatPanorama::set_disable_fixed_width(bool v) {
    auto* storage =
            dynamic_cast<ArrayInvertedListsPanorama*>(invlists);
    FAISS_THROW_IF_NOT_MSG(
            storage,
            "IndexIVFFlatPanorama requires ArrayInvertedListsPanorama");
    storage->pano->disable_fixed_width = v;
}
