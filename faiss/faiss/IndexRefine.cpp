...
/// Walk through any IndexPreTransform wrappers to find an underlying
/// IndexFlatPanorama, if present. Returns nullptr if not found.
IndexFlatPanorama* find_inner_panorama(Index* idx) {
    while (idx) {
        if (auto* pano = dynamic_cast<IndexFlatPanorama*>(idx)) {
            return pano;
        }
        if (auto* pre = dynamic_cast<IndexPreTransform*>(idx)) {
            idx = pre->index;
        } else {
            return nullptr;
        }
...
void IndexRefinePanorama::search(
    ...
    // Apply the params->epsilon to the inner FlatL2Panorama via
    // set_epsilon() before invoking search_subset(). search_subset() does
    // not take SearchParameters in the current Index API, so we pass
    // epsilon by mutation. This is contained inside the search call
    // (no external mutation needed by callers). A future cleanup is to
    // extend search_subset to take SearchParameters*.
    if (params != nullptr) {
        IndexFlatPanorama* pano = find_inner_panorama(refine_index);
        if (pano != nullptr) {
            pano->set_epsilon(params->epsilon);
        }
    }
