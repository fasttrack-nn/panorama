...
struct IndexHNSWFlatPanorama : IndexHNSWFlat {
    ...
    /// Pointer to the cum-sums of vector `i` in the underlying interleaved
    /// storage. Convenience wrapper around the storage helper used on the
    /// HNSW-Panorama search hot path.
    const float* get_cum_sum(idx_t i) const;

    /// Pointer to the vector data of vector `i` in the underlying
    /// interleaved storage (i.e. one cum-sums prefix past the row start).
    const float* get_inline_xb(idx_t i) const;

    /// Number of floats per row in the underlying interleaved storage
    /// (cum-sums prefix + vector data). Used as the row stride for SIMD
    /// kernels reading from `storage->codes`.
    size_t inline_row_floats() const;
