...
struct IndexIVFFlatPanorama : IndexIVFFlat {
    ...
    /// Ablation: switch between the optimized kernel (default, false) and
    /// the unoptimized fused-loop kernel (true). Forwards to
    /// PanoramaFlat::use_unoptimized_kernel; see Panorama.h for the
    /// algorithmic difference.
    void set_use_unoptimized_kernel(bool v);

    /// Ablation: when true (and use_unoptimized_kernel is false), the
    /// optimized kernel skips its with_level_width compile-time dispatch
    /// and always invokes compute_level_dot_products_flat<Direct,
    /// FixedWidth=0>. Forwards to PanoramaFlat::disable_fixed_width.
    void set_disable_fixed_width(bool v);
};
