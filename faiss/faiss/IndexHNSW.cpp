...
IndexHNSWFlatPanorama::IndexHNSWFlatPanorama(
    ...
    // Initialize the Index/IndexHNSW state that the default ctor left
    // empty, plus our row-interleaved [cum_sums | xb] storage.
    d = d_in;
    metric_type = metric;
    hnsw = HNSW(M);
    storage = new IndexFlatPanoramaInline(d_in, num_panorama_levels_in);
...
const float* IndexHNSWFlatPanorama::get_cum_sum(idx_t i) const {
    return static_cast<const IndexFlatPanoramaInline*>(storage)->get_cum_sum(i);
}

const float* IndexHNSWFlatPanorama::get_inline_xb(idx_t i) const {
    return static_cast<const IndexFlatPanoramaInline*>(storage)
            ->get_inline_xb(i);
}

size_t IndexHNSWFlatPanorama::inline_row_floats() const {
    return static_cast<const IndexFlatPanoramaInline*>(storage)->row_floats();
}
