    ...
    def test_ratio_dims_scanned(self):
        ...
        for metric in self.METRICS:
            with self.subTest(metric=metric):
                ...
                # The hyper-optimized refine path processes candidates in
                # mini-batches (REFINE_BLOCK == 64 in IndexFlat.cpp), so the
                # heap threshold only tightens between blocks, not after
                # every single candidate. That can scan a few extra dims
                # at the level boundary -- bounded by REFINE_BLOCK / k_base.
                # We allow ~3% relative slack to absorb that.
                np.testing.assert_allclose(
                    ratios,
                    expected_ratios,
                    atol=2e-3,
