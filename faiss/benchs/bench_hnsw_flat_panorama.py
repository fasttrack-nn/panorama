...
if __name__ == "__main__":
    ...
    datasets = [
        (DatasetSIFT1M(), "SIFT1M", 4),
        (DatasetGIST1M(), "GIST1M", 8),
        # Synthetic high-dimensional dataset: 2048d, 100k train, 1M database, 10k queries
        (SyntheticDataset(2048, 100000, 1000000, 10000), "Synthetic2048D", 8),
    ]

    for ds, name, ds_nlevels in datasets:
        print(f"\n{'='*60}")
        print(f"Benchmarking on {name}")
        print(f"{'='*60}")
        benchmark_dataset(ds, name, k=k, nlevels=ds_nlevels, M=M)
